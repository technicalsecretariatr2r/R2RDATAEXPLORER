...
st.markdown('<div style="text-align: justify;">RR2R is a worldwide campaign launched in 2021 by the UN Climate Change High-Level Champions (HLCs). It aims to increase the resilience of four billion people living in vulnerable communities in collaboration with partner organizations from around the world, while developing tools to support them in their work. The campaign has developed a people-centred resilience Metrics Framework for non-State actors to report climate resilience actions and to quantify and validate their impact under a common framework. This framework will be open for public consultation at COP27.</div>', unsafe_allow_html=True)
st.markdown('<div style="text-align: justify;"></div>', unsafe_allow_html=True) ##Espacio Texto
st.markdown('<div style="text-align: justify;"The Metrics Framework pursues several objectives at once, playing an essential function for both the campaign and the global climate action agenda and resilience communities. The Framework is the cornerstone of the R2R campaign, serving as a guide, for partners and for the HLCs, on how to manage and develop their work. It is composed of two complementary sets of metrics: Quantitative or Magnitude Metrics, which help estimate the size of the impact, fundamentally through the number of beneficiaries reached; and Qualitative or Depth Metrics, which help provide an understanding of how the partners and their members are contributing to increasing resilience of people vulnerable to climate change, by observing which conditions (Resilience Attributes) they are impacting. </div>', unsafe_allow_html=True)
st.markdown('<div style="text-align: justify;"></div>', unsafe_allow_html=True) ##Espacio Texto
